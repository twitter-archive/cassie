...
import com.twitter.cassie.{Column, ColumnFamily, RowIteratee}
...
class ColumnIteratorTest extends Spec with MustMatchers with MockitoSugar with OneInstancePerTest {
  ...
  describe("iterating through an empty column family") {
    ...
    val iterator = new RowIteratee(cf, b("start"), b("end"), 5, predicate, Utf8Codec, Utf8Codec, Utf8Codec).iterator()
