...
class VarLongCodecTest extends Spec with MustMatchers {
  describe("encoding a long") {
    it("produces a variable length zig-zag encoded array of bytes") {
      VarLongCodec.encode(199181989101092820L).toList must equal(List[Byte](-88, -1, -75, -96, -41, -67, -47, -61, 5))
    }
  }

  describe("decoding an array of bytes") {
    it("produces a long") {
      VarLongCodec.decode(Array(-88, -1, -75, -96, -41, -67, -47, -61, 5)) must equal(199181989101092820L)
    }
  }
}
